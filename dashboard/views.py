...
def student_course_detail(request, course_id):
    ...
    materials = CourseMaterial.objects.filter(course=course).order_by('-uploaded_at')[:5]  # Only recent materials
    ...
    # Calculate downloaded count for progress display - ONLY COUNT REQUIRED MATERIALS
    total_required_materials = CourseMaterial.objects.filter(
        course=course,
        is_required=True
    ).count()
    ...
    downloaded_count = MaterialDownload.objects.filter(
        ...
        material__is_required=True  # Only count required materials
    ).count()

    progress_percentage = 0
    if total_required_materials > 0:
        progress_percentage = int((downloaded_count / total_required_materials) * 100)

    # Get total materials count for display (all materials, not just required)
    total_all_materials = CourseMaterial.objects.filter(course=course).count()

    # Get downloaded count for all materials (for the "X of Y materials downloaded" display)
    downloaded_all_count = MaterialDownload.objects.filter(
        student=student,
        material__course=course
    ).count()
    ...
    context = {
        ...
        'materials': materials,
        'progress': progress,
        'downloaded_count': downloaded_count,  # Required materials downloaded
        'total_materials': total_required_materials,  # Total required materials
        'progress_percentage': progress_percentage,
        'downloaded_all_count': downloaded_all_count,  # All materials downloaded
        'total_all_materials': total_all_materials,  # All materials total
    }
...
# Update the student_course_detail view
@login_required
def student_course_detail(request, course_id):
    if not hasattr(request.user, 'student'):
        return redirect('dashboard')
    ...
    # FIX: Count ALL materials for progress calculation, not just required ones
    total_materials = CourseMaterial.objects.filter(course=course).count()

    # FIX: Count downloads for ALL materials
    downloaded_count = MaterialDownload.objects.filter(
        student=student,
        material__course=course
    ).count()
